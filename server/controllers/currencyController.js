...
exports.getRates = async (req, res) => {
  try {
    ...
    const { data } = await axios.get(`https://open.er-api.com/v6/latest/${base.toUpperCase()}`);
...
exports.convertCurrency = async (req, res) => {
  try {
    ...
    const { data } = await axios.get(`https://open.er-api.com/v6/latest/${from.toUpperCase()}`);
    if (data.result !== 'success') {
      return res.status(400).json({ message: 'Invalid currency code' });
    }

    const rate = data.rates[to.toUpperCase()];
    if (!rate) return res.status(400).json({ message: 'Target currency not found' });
